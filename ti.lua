-- Global aimbot settings stored using getgenv()
getgenv().aimbot = {
    Enabled = true,
...
-- Function to get the closest part to aim at
getgenv().aimbot.GetClosestPart = function()
    local parts, target = {}, nil
    for _, v in ipairs(getgenv().aimbot.CustomParts) do
    ...
    for _, v in ipairs(parts) do
        ...
        if dist <= getgenv().aimbot.FOV and pos.Z > 0 and (not target or dist < target.Distance) then
...
-- Detect key press and release for aimbot activation
UIS.InputBegan:Connect(function(input)
    if input.KeyCode == getgenv().aimbot.Key or input.UserInputType == getgenv().aimbot.Key then
        keypressed = true
    end
end)
...
-- Update FOV circle size and position
...
-- Aimbot loop for snapping to the target
RS.RenderStepped:Connect(function()
    if getgenv().aimbot.Enabled and keypressed then
        local part = getgenv().aimbot.GetClosestPart()
        if part then
            getgenv().aimbot.Aim(part.Position.X, part.Position.Y)
        end
    end
end)
